  ...
  return (
    <Card className="shadow-sm hover:shadow-md transition-shadow">
      <CardHeader>
        ...
        <CardDescription className="font-body text-sm">Case ID: {document.caseId} | Version: {document.version}</CardDescription>
      </CardHeader>
      <CardContent>
        <p className="font-body text-xs text-muted-foreground">Uploaded: {new Date(document.uploadedDate).toLocaleDateString()}</p>
      </CardContent>
      <CardFooter className="flex justify-end gap-2">
        <Button variant="outline" size="sm" className="font-body" onClick={() => alert('View document: ' + document.name)}>
          <Eye className="mr-2 h-4 w-4" /> View
        </Button>
        <Button variant="outline" size="sm" className="font-body" onClick={() => alert('Download document: ' + document.name)}>
        ...
        {document.status === "Awaiting Signature" && (
          <Button size="sm" onClick={() => onSign(document)} className="font-body bg-accent hover:bg-accent/90 text-accent-foreground">
            <PenSquare className="mr-2 h-4 w-4" /> Sign Document
          </Button>
        )}
      </CardFooter>
    </Card>
